...
new class extends Component {
    ...
    #[\Livewire\Attributes\Url(as:'semester')] public string $semesterId='';
    #[\Livewire\Attributes\Url(as:'kelas')] public string $kelasId='';
    #[\Livewire\Attributes\Url(as:'mata_pelajaran')] public string $mataPelajaranId='';
    #[\Livewire\Attributes\Url] public string $tingkat='';
    public string $pengajaranTahunAjaranId='';
    public function mount(?string $resource=null): void { $this->resource=$resource??request()->route('resource'); abort_unless(isset($this->definitions()[$this->resource]),404);if(in_array($this->resource,['kelas','pengajaran'],true)){$validYears=TahunAjaran::whereKey($this->tahunAjaranId)->exists();if(!request()->query->has('tahun_ajaran')||(!$validYears&&$this->tahunAjaranId!==''))$this->tahunAjaranId=(string)(TahunAjaran::aktif()->value('id')??'');}if($this->resource==='kelas'&&!in_array($this->tingkat,['','1','2','3','4','5','6'],true))$this->tingkat='';if($this->resource==='pengajaran')$this->validatePengajaranFilterState();$this->initializeDataTable(); }
    public function updatedStatus(): void{$this->datasetChanged();}
    public function updatedTahunAjaranId():void{if($this->resource==='pengajaran'){$this->semesterId='';$this->kelasId='';}$this->datasetChanged();}
    public function updatedSemesterId():void{$this->datasetChanged();}
    public function updatedKelasId():void{$this->datasetChanged();}
    public function updatedMataPelajaranId():void{$this->datasetChanged();}
    public function updatedTingkat():void{$this->datasetChanged();}
    public function updatedPengajaranTahunAjaranId():void{$this->form['semester_id']='';$this->form['kelas_id']='';$this->resetValidation(['form.semester_id','form.kelas_id','pengajaranTahunAjaranId']);}
    public function resetTableFilters():void{$this->status='';$this->tahunAjaranId=in_array($this->resource,['kelas','pengajaran'],true)?(string)(TahunAjaran::aktif()->value('id')??''):'';$this->semesterId='';$this->kelasId='';$this->mataPelajaranId='';$this->tingkat='';}
    private function definitions(): array { return [
        ...
        'kelas'=>['label'=>'Kelas','model'=>Kelas::class,'permission'=>'kelas.create','search'=>['nama'],'with'=>['tahunAjaran','waliKelas'],'sort'=>['tahun_ajaran_sort','nama','tingkat','wali_kelas_sort','jumlah_siswa','kelas.status'],'columns'=>['tahun_ajaran_label','nama','tingkat','wali_kelas_label','jumlah_siswa','status'],'columnSort'=>['tahun_ajaran_label'=>'tahun_ajaran_sort','wali_kelas_label'=>'wali_kelas_sort','status'=>'kelas.status'],'columnLabels'=>['tahun_ajaran_label'=>'Tahun Ajaran','wali_kelas_label'=>'Wali Kelas','jumlah_siswa'=>'Jumlah Siswa'],'fields'=>[
    ...
    public function pengajaranSemesterOptions(?string $tahunAjaranId=null):array{return Semester::query()->with('tahunAjaran')->when($tahunAjaranId,fn($q,$id)=>$q->where('tahun_ajaran_id',$id))->orderByDesc('tanggal_mulai')->get()->mapWithKeys(fn($x)=>[$x->id=>$x->tahunAjaran->nama.' · '.ucfirst($x->nama)])->all();}
    public function pengajaranKelasOptions(?string $tahunAjaranId=null):array{return Kelas::query()->with('tahunAjaran')->where('status','aktif')->when($tahunAjaranId,fn($q,$id)=>$q->where('tahun_ajaran_id',$id))->orderBy('tingkat')->orderBy('nama')->get()->mapWithKeys(fn($x)=>[$x->id=>$x->nama.' · '.$x->tahunAjaran->nama])->all();}
    private function validatePengajaranFilterState():void{if($this->semesterId&&!Semester::whereKey($this->semesterId)->when($this->tahunAjaranId,fn($q)=>$q->where('tahun_ajaran_id',$this->tahunAjaranId))->exists())$this->semesterId='';if($this->kelasId&&!Kelas::whereKey($this->kelasId)->when($this->tahunAjaranId,fn($q)=>$q->where('tahun_ajaran_id',$this->tahunAjaranId))->exists())$this->kelasId='';if($this->mataPelajaranId&&!MataPelajaran::whereKey($this->mataPelajaranId)->exists())$this->mataPelajaranId='';}
    public function openCreate(): void { abort_unless(auth()->user()->can($this->definition()['permission']),403); $this->resetValidation();$this->editingId=null;$this->form=[];foreach($this->definition()['fields'] as $key=>$field)$this->form[$key]=$key==='status'?'aktif':'';if($this->resource==='pengajaran')$this->pengajaranTahunAjaranId='';$this->showForm=true; }
    public function edit(int $id): void { abort_unless(auth()->user()->can(str_replace('create','update',$this->definition()['permission']))||auth()->user()->can($this->definition()['permission']),403);$item=$this->definition()['model']::findOrFail($id);$this->editingId=$id;$this->form=$item->only(array_keys($this->definition()['fields']));foreach($this->definition()['fields'] as $key=>$field)if($field['type']==='date'&&$item->{$key})$this->form[$key]=$item->{$key}->format('Y-m-d');if($this->resource==='pengguna'){$this->form['role']=$item->getRoleNames()->first();$this->form['password']='';}if($this->resource==='pengajaran')$this->pengajaranTahunAjaranId=(string)$item->semester()->value('tahun_ajaran_id');$this->showForm=true; }
...
<div class="mb-4 grid gap-3 {{ $resource==='pengajaran'?'':'lg:grid-cols-[minmax(18rem,1fr)_auto]' }}">
    <div class="card p-4">
    ...
    <div class="card flex flex-wrap items-end gap-3 p-4">
        ...
        @endif
        @if($resource==='pengajaran')
            <div class="min-w-48 flex-1"><label class="form-label">Filter Tahun Ajaran</label><x-searchable-select model="tahunAjaranId" :value="$tahunAjaranId" :options="$this->options('tahun')" placeholder="Semua tahun ajaran" search-placeholder="Cari tahun ajaran..." /></div>
            <div class="min-w-48 flex-1"><label class="form-label">Filter Semester</label><x-searchable-select model="semesterId" :value="$semesterId" :options="$this->pengajaranSemesterOptions($tahunAjaranId)" placeholder="Semua semester" search-placeholder="Cari semester..." /></div>
            <div class="min-w-48 flex-1"><label class="form-label">Filter Kelas</label><x-searchable-select model="kelasId" :value="$kelasId" :options="$this->pengajaranKelasOptions($tahunAjaranId)" placeholder="Semua kelas" search-placeholder="Cari kelas..." /></div>
            <div class="min-w-48 flex-1"><label class="form-label">Filter Mata Pelajaran</label><x-searchable-select model="mataPelajaranId" :value="$mataPelajaranId" :options="$this->options('mapel')" placeholder="Semua mata pelajaran" search-placeholder="Cari mata pelajaran..." /></div>
        @endif
...
@if($resource==='pengajaran')
    <div><label class="form-label">Tahun Ajaran <span>*</span></label><x-searchable-select model="pengajaranTahunAjaranId" :value="$pengajaranTahunAjaranId" :options="$this->options('tahun')" placeholder="Pilih tahun ajaran dahulu..." search-placeholder="Cari tahun ajaran..." />@error('pengajaranTahunAjaranId')<p class="form-error" role="alert">{{ $message }}</p>@enderror</div>
@endif
...
@if($resource==='pengajaran'&&in_array($key,['semester_id','kelas_id'],true))
    @if($pengajaranTahunAjaranId)
        @php $dependentOptions=$key==='semester_id'?$this->pengajaranSemesterOptions($pengajaranTahunAjaranId):$this->pengajaranKelasOptions($pengajaranTahunAjaranId); @endphp
        <x-searchable-select model="form.{{ $key }}" :value="$form[$key]??''" :options="$dependentOptions" placeholder="Pilih {{ strtolower($field['label']) }}..." search-placeholder="Cari {{ strtolower($field['label']) }}..." />
    @else
        <button type="button" disabled class="form-input cursor-not-allowed text-left text-slate-400">Pilih tahun ajaran terlebih dahulu</button>
    @endif
@elseif(($field['searchable']??false)===true)@php $opts=$field['values']??$this->options($field['options']??''); @endphp<x-searchable-select model="form.{{ $key }}" :value="$form[$key]??''" :options="$opts" placeholder="Pilih {{ strtolower($field['label']) }}..." search-placeholder="Cari {{ strtolower($field['label']) }}..." />
