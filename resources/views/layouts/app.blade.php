...
@php
    ...
    if ($role !== 'admin' && request()->routeIs('dashboard')) {
        $pageTitle = 'Dashboard ' . str($roleLabel)->title();
    }
@endphp
...
<body class="sisdar-app" x-data="{ sidebarOpen: false }" @keydown.escape.window="sidebarOpen = false">
    ...
    <div class="app-shell">
        <div x-show="sidebarOpen" x-transition.opacity x-cloak class="app-overlay" @click="sidebarOpen=false"
        ...
        <aside id="sidebar" class="app-sidebar" :class="sidebarOpen && 'open'" aria-label="Navigasi utama">
            <div class="sidebar-brand">
                ...
                <button type="button" @click="sidebarOpen=false" aria-label="Tutup menu"><svg viewBox="0 0 24 24"
            ...
            <nav class="sidebar-nav">
                ...
                @endrole
                @role('guru')
                    <x-nav-group label="Proses Belajar"><x-nav-link route="jadwal.index" label="Jadwal Mengajar"
                            icon="schedule" /><x-nav-link route="nilai.index" label="Input & Riwayat Nilai"
                            icon="grade" /></x-nav-group>
                @endrole
                @role('siswa')
                    <x-nav-group label="Akademik"><x-nav-link route="jadwal.index" label="Jadwal Pelajaran"
                            icon="schedule" /><x-nav-link route="nilai.index" label="Nilai Saya"
                            icon="grade" /></x-nav-group>
                @endrole
                @role('kepala_sekolah')
                    <x-nav-group label="Monitoring"><x-nav-link route="jadwal.index" label="Jadwal Pelajaran"
                            icon="schedule" /><x-nav-link route="nilai.index" label="Nilai Siswa"
                            icon="grade" /><x-nav-link route="laporan.jadwal" label="Laporan"
                            icon="report" /><x-nav-link route="aktivitas.index" label="Aktivitas"
                            icon="schedule" /></x-nav-group>
                @endrole
                @unlessrole('admin')
                    <x-nav-group label="Akun">
                        @unlessrole('kepala_sekolah')
                            <x-nav-link route="aktivitas.index" label="Aktivitas Saya" icon="schedule" />
                        @endunlessrole
                        <x-nav-link route="profile.edit" label="Profil" icon="profile" /></x-nav-group>
                @endunlessrole
            </nav>
            <div class="sidebar-help"><a href="{{ route('bantuan') }}" target="_blank" rel="noopener"><svg
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                        <circle cx="12" cy="12" r="9" />
                        <path d="M9.7 9a2.5 2.5 0 1 1 3.4 2.34c-.72.33-1.1.77-1.1 1.66M12 17h.01" />
                    </svg>Bantuan</a></div>
        </aside>

        <div class="app-workspace">
            <header class="app-header">
                <button type="button" class="mobile-menu" @click="sidebarOpen=true" aria-controls="sidebar"
                    aria-label="Buka menu"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2">
                        <path d="M4 7h16M4 12h16M4 17h16" />
                    </svg></button>
                <div class="header-title">
                    <h1>{{ $pageTitle }}</h1>
                    <p>{{ request()->routeIs('dashboard') ? 'Selamat datang, ' . auth()->user()->name . '. Kelola data sekolah dengan mudah.' : 'Kelola informasi SISDAR dengan mudah dan terstruktur.' }}
                    </p>
                </div>
                <div class="header-tools">
                    <a href="{{ route('home') }}" class="header-home"
                        aria-label="Kembali ke halaman awal"><x-nav-icon name="home" /><span>Halaman
                            Awal</span></a>
                    <div class="header-account" x-data="{ open: false }" @click.outside="open = false">
                        <button type="button" class="header-profile" @click="open = !open"
                            :aria-expanded="open.toString()" aria-haspopup="menu" aria-controls="account-menu">
                            <span>{{ str(auth()->user()->name)->substr(0, 1)->upper() }}</span>
                            <div><strong>{{ auth()->user()->name }}</strong><small>{{ $roleLabel }}</small></div>
                            <svg :class="open && 'rotate'" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="1.8" aria-hidden="true">
                                <path d="m8 10 4 4 4-4" />
                            </svg>
                        </button>
                        <div id="account-menu" x-show="open" x-transition.origin.top.right x-cloak
                            class="account-dropdown" role="menu">
                            ...
                            <a href="{{ route('profile.edit') }}" wire:navigate @click="open = false"
                                role="menuitem"><x-nav-icon name="profile" />Profil Saya</a>
                            <a href="{{ route('bantuan') }}" target="_blank" rel="noopener" @click="open = false"
                ...
                <div class="academic-selector"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    ...
                    <strong>{{ app(\App\Services\PeriodeAktif::class)->tahunAjaran()?->nama ?? 'Belum Aktif' }}</strong><svg
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                        <path d="m8 10 4 4 4-4" />
                    </svg></div>
            </header>
    ...
    <div x-data="{ show: false, type: 'success', message: '', timer: null }"
        ...
        x-show="show" x-transition x-cloak class="toast" :class="type"
        :role="type === 'error' ? 'alert' : 'status'" x-text="message"></div>
    @if (session('success'))
